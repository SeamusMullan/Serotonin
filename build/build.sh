...
if [ -x "$DIR/../build-tools/cross/bin/i686-serotonin-gcc" ]; then
    export PREFIX="$DIR/../build-tools/cross"
    export TARGET=i686-serotonin
elif [ -x "$DIR/../build-tools/bin/bin/i686-elf-gcc" ]; then
...
# da compiler
$CC -c audio/pcspeaker/pcspeaker.c -o audio/pcspeaker/pcspeaker.o $CFLAGS
$CC -c audio/opl2/opl2.c -o audio/opl2/opl2.o $CFLAGS
$CC -c audio/startup/opl2_sound/opl2_startup.c -o audio/startup/opl2_sound/opl2_startup.o $CFLAGS
...
$CC -T linker.ld -o ../build/serotonin.bin -ffreestanding -O2 -nostdlib boot.o kernel.o tty.o string.o stdlib/stdlib.o stdio/stdio.o stdlib/mem.o gdt.o idt.o isr.o fault.o io/irq.o io/pic.o io/keyboard.o vmm/paging_init.o vmm/vmm.o video/vbe/vbe.o video/font.o filesystem/vfs.o filesystem/devfs/devfs.o filesystem/tmpfs/tmpfs.o device/ide/ide_pci.o filesystem/fat32/fat32.o filesystem/blkcache.o schedule/schedule.o schedule/switch_task.o audio/pcspeaker/pcspeaker.o audio/opl2/opl2.o syscall/isr_syscall.o syscall/syscall.o audio/startup/opl2_sound/opl2_startup.o schedule/kernel_yield.o schedule/signal_trampoline.o filesystem/vfs_perm.o filesystem/user_fs/user_fs.o device/devfs_example.o io/rtc.o io/serial.o io/pci/pci.o device/mouse/dev_mouse.o device/keyboard/dev_keyboard.o device/serial/dev_serial.o device/rtl8139/rtl8139.o device/rtl8139/dev_rtl8139.o device/ac97/ac97.o device/ac97/dev_ac97.o device/pci_drivers.o pty/pty.o $TEST_OBJS -lgcc
